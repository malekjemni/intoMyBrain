...
public class MapManager : MonoBehaviour
{
    ...
    private void CleanUpPath(List<Cell> path)
    {
        ...
        Instantiate(endPoint, Pathway[Pathway.Count-1], Quaternion.identity);
        GameObject marker = Instantiate(startMarker, Pathway[0], Quaternion.identity);
        GameObject marker2 = Instantiate(endMarker, Pathway[Pathway.Count - 1], Quaternion.identity);
        Vector3 direction = Pathway[1] - Pathway[0];
        Vector3 direction2 = Pathway[Pathway.Count - 1] - Pathway[Pathway.Count - 2];
        Quaternion rotation = Quaternion.LookRotation(direction, Vector3.up);
        Quaternion rotation2 = Quaternion.LookRotation(direction2, Vector3.up);
        Instantiate(arrow, Pathway[0], rotation);
        Instantiate(arrow, Pathway[Pathway.Count - 2], rotation2);

        marker2.transform.position = new Vector3(Pathway[Pathway.Count - 1].x, 1f, Pathway[Pathway.Count - 1].z);
        marker.transform.position = new Vector3(Pathway[0].x, 0.1f, Pathway[0].z);
    }
