...
public class TowerBase : MonoBehaviour, IBuildable, IInspectable
{
    ...
    [SerializeField] public string loreDescription;
    ...
    public TowerType MyTowerType => towerType;
    ...
    protected virtual void FixedUpdate()
    {
        ...
        for (int i = 0; i < colliders.Length; i++)
        {
            if (HasLOSToTarget(colliders[i].transform, out RaycastHit losObstruct))
            {
                switch (towerType)
                {
                    case TowerType.Friendly:
                        if (colliders[i].gameObject.CompareTag("Friendly"))
                        {
                            targetsInRange.Add(colliders[i].transform);
                        }
                        break;
                    case TowerType.Hostile:
